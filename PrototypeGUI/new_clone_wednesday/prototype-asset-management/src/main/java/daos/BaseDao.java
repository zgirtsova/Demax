...
public abstract class BaseDao<T> {
        ...
        try {
            ...
            if (rs.next()) {
                for (int i = 1; i <= meta.getColumnCount(); i++) {
                    ...
                    try {
                        Method setter = this.fieldsByColumnName.get(columnName).getSetter();
                        if (setter.getParameterTypes()[0].equals(LocalDateTime.class)) {
                            setter.invoke(this.instanceOfClazz, ((Timestamp) rs.getObject(i)).toLocalDateTime());
                        } else {
                            setter.invoke(this.instanceOfClazz, rs.getObject(i));
                        }
    ...
    private void collectFieldsData(Field[] fields, Method[] methods) {
        ...
        for (Field field : fields) {
            ...
            String columnName = field.getAnnotation(ColumnName.class).columnName();
