...
public class JLibImpl<K, V> implements JLibMap<K, V> {
    ...
    public void saveMap() {
        try
        {
            FileOutputStream fos =
                    new FileOutputStream(Paths.JLIBMAP_FILE_NAME);
            ObjectOutputStream oos = new ObjectOutputStream(fos);
            oos.writeObject(this.innerMap);
            oos.close();
            fos.close();
            System.out.printf("Serialized HashMap data is saved in hashmap.ser %n");
    ...
    public static JLibMap<String, Object> readMapFromFile() {
        ...
        try {
            FileInputStream fis = new FileInputStream(Paths.JLIBMAP_FILE_NAME);
